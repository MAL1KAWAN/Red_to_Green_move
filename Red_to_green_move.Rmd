...
#### Lets visualize Win/loss frequency of the days where stock opened below the previous close and fit our model. 
```{r}



glm.fits <- glm(filter_data$`Win/Loss`~ filter_data$Percent_Difference + filter_data$RSI_ind,family="binomial")
summary(glm.fits)
...
table(glm.pred, filter_data$`Win/Loss`)
...
FAANG <- function(ticker){
  ...
  stock_data <- stock_data[,c(6,14,15,16,17,18,19,20)]
  return(stock_data)
}
...
ggplot(Amazon, aes(`Win/Loss`)) +
  geom_bar(fill = "#0073C2FF")

table(filter_data$`Win/Loss`)

glm.fits <- glm(`Win/Loss`~. -MA50-MA200-RSI_ind-Volume, data = facebook ,family="binomial")
summary(glm.fits)
